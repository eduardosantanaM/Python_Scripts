...
parser.add_argument("stored_zips", type=str, help="Pasta permanente dos zips (os zips serao salvos aqui)")
    ...
    while True:
        ...
        for zip_file in zip_files:
            ...
            if response.lower() == 's':
                ...
                store_response = input(f"Voce quer guardar o arquivo {zip_file} em {stored_zips_folder} folder? (s/n): ").lower()
                if store_response.lower() == 's':
                    copy_to_restored_zips(stored_zips_folder, full_zip_path, zip_file)
                db_name = input(f"Qual será o nome do banco de dados? ").lower()
                process_mongo_restore_routine(db_name, full_zip_path, 
                                              host, port,
                                              backup_folder,
                                              new_password_hash,
                                              zip_file)
