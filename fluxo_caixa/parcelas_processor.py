...
class ParcelasProcessor:
    def __init__(self, mongo_uri:str, db_name, conta_id, data_inicio, data_fim, page_size=1000, conta: Dict[str, Any] | None=None):
            ...
            for parcela in parcelas:
                ...
                if (parcela.get('ValorLiquido', 0) > 0 and parcela['ValorLiquido'] != valor_rateio_positivo) or (parcela.get('ValorLiquido', 0) == 0 and valor_rateio_positivo != parcela['ValorBruto']):
                    ...
                    self.divergencias.append({
                        ...
                        # Verifica se "Liquidacao" é um datetime antes de usar strftime
                        'liquidacao': parcela["Liquidacao"].strftime('%Y-%m-%d') if isinstance(parcela["Liquidacao"], datetime) else parcela["Liquidacao"],

                        'nsu': parcela.get('Cartao', {}).get('Obj', {}).get('Chave', [{}])[0].get('NSU_Host', ''),
                        
                        # Verifica se "DataDaVenda" é um datetime antes de usar strftime
                        'data_venda': parcela.get('Cartao', {}).get('Obj', {}).get('Chave', [{}])[0].get('DataDaVenda', '').strftime('%Y-%m-%d') 
                                    if isinstance(parcela.get('Cartao', {}).get('Obj', {}).get('Chave', [{}])[0].get('DataDaVenda', ''), datetime)
                                    else parcela.get('Cartao', {}).get('Obj', {}).get('Chave', [{}])[0].get('DataDaVenda', ''),
