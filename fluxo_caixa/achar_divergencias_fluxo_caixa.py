...
api = FluxoDeCaixaAPI(base_url="http://localhost:1337", headers=headers)


# Função para conectar ao MongoDB e obter resultados de uma coleção
def conectar_mongo_e_obter_resultados(
    uri: str, database_name: str, collection_name: str
) -> List[Dict[str, Any]]:
    try:
        ...
        print(
            f"Erro ao conectar ao MongoDB ou obter resultados da coleção {collection_name}: {e}"
        )
        return []
...
def extrair_saldo_final_analitico(
    dados_fluxo_analitico: Dict[str, Any]
) -> Optional[float]:
    try:
...
def main():
    ...
    contas: List[Dict[str, Any]] = conectar_mongo_e_obter_resultados(
    ...
    for conta in contas_filtradas:
        ...
        while data_atual <= data_fim:
            print(
                f"Verificando fluxos de caixa para a data: {data_atual.strftime('%Y-%m-%d')}"
            )

            # Usar a classe FluxoDeCaixaAPI para fazer as requisições
            dados_fluxo_caixa_simples = api.requisitar_fluxo_de_caixa_simples(
                data_atual, data_atual, conta_id
            )
            dados_fluxo_caixa_analitico = api.requisitar_fluxo_de_caixa_analitico(
                data_atual, data_atual, conta_id
            )
            ...
            if dados_fluxo_caixa_simples and dados_fluxo_caixa_analitico:
                ...
                saldo_analitico = extrair_saldo_final_analitico(
                    dados_fluxo_caixa_analitico
                )
                ...
                if saldo_simples is not None and saldo_analitico is not None:
                    ...
                    if diferenca != 0:
                        print(
                            f"Diferença encontrada para a conta {conta['Nome']} no dia {data_atual.strftime('%Y-%m-%d')}: {diferenca}"
                        )

                        # Busca detalhes das parcelas com erro
                        data_inicio_busca_db = data_atual.replace(
                            hour=0, minute=0, second=0, microsecond=0
                        )
                        data_final_busca_db = (data_atual + timedelta(days=1)).replace(
                            hour=0, minute=0, second=0, microsecond=0
                        )
                        ...
                        print(
                            f"Nenhuma diferença encontrada para a conta {conta['Nome']} no dia {data_atual.strftime('%Y-%m-%d')}."
                        )
                else:
                    print(
                        f"Erro ao extrair saldos para a conta {conta['Nome']} no dia {data_atual.strftime('%Y-%m-%d')}."
                    )
            else:
                print(
                    f"Erro ao recuperar dados para a conta {conta['Nome']} no dia {data_atual.strftime('%Y-%m-%d')}."
                )
